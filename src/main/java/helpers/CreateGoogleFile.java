...
//        java.io.File f = new java.io.File("C:/tmp/1.png");
//        System.out.println(Files.probeContentType(f.toPath()));
        java.io.File uploadFile = new java.io.File("C:/tmp/1.png");

        // Create Google File:

        File googleFile = createGoogleFile(null, "image/png", "newfile.txt", uploadFile);
    ...
    public static ArrayList<String> uploadAndGetURLs(ArrayList<java.io.File> files) {
        ArrayList<String> outputURLs = new ArrayList<String>();
        for(java.io.File f : files) {
            try {
                File googleFile = createGoogleFile(null, "image/png", ""+Math.random()*999999+".txt", f);
                outputURLs.add(googleFile.getWebViewLink());
            } catch(Exception e) {
                e.printStackTrace();
            }
        }
